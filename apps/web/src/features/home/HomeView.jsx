...
export function HomeView({ state, updateState, handleSendMessage, handleGenerateImage, chatFileInputRef }) {
    // Голосовой ввод: финальный текст добавляется в поле; во время диктовки
    // показываем «живую» реакцию (interim) прямо в поле ввода.
            ...
            <div className="px-6 pt-16 sm:pt-20 max-w-4xl mx-auto">
                ...
                <div className="void-input-rise relative max-w-4xl mx-auto pointer-events-auto mb-10">
                    ...
                    {/* Индикация записи — отдельной строкой НАД полем ввода,
                        чтобы не перекрывать уже набранный текст */}
                    {voice.listening && (
                    ...
                    <div className="flex items-end bg-white dark:bg-darkCard rounded-3xl border border-gray-200 dark:border-darkBorder shadow-md focus-within:ring-4 focus-within:ring-[#5b32d4]/10 focus-within:border-[#5b32d4] transition-all relative">
                        ...
                        {!state.inputValue && !voice.listening && (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 sm:gap-4 md:gap-5 mb-8 sm:mb-10">
                    ...
                    <div onClick={() => updateState({currentView: 'agent-store'})} className="bg-white dark:bg-darkCard p-3.5 sm:p-5 md:p-6 rounded-2xl md:rounded-[2rem] border border-gray-100 dark:border-darkBorder shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                        ...
                        <h3 className="font-bold text-sm sm:text-base md:text-lg mb-0.5 sm:mb-1 dark:text-white">Магазин агентов</h3>
                        <p className="hidden sm:block text-sm text-gray-500 dark:text-gray-400 font-medium">Готовые агенты и оркестраторы.</p>
                    </div>
