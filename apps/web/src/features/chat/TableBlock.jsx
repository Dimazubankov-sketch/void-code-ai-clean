...
// в едином стиле проекта (тёмный фон, скруглённые углы, полосы через
// строку). Горизонтальный скролл на узких экранах — чтобы длинные таблицы
...
function renderCell(text, keyPrefix) {
    if (!text) return null;
    // Простейший разбор **bold** — как в MessageRenderer, без dangerouslySetInnerHTML
    ...
    return (
        <div ref={rootRef} className="my-3 rounded-2xl overflow-hidden border border-gray-800 bg-[#0f0f1a] shadow-sm">
            <div className="overflow-x-auto">
                <table className="w-full text-[13px] text-gray-200 border-collapse">
                    <thead>
                        <tr className="bg-[#181828] border-b border-gray-800">
                            {Array.from({ length: columnCount }).map((_, i) => (
                                <th
                                    key={i}
                                    className="px-3 py-2.5 font-semibold text-gray-100 whitespace-nowrap"
                    ...
                    <tbody>
                        {bodyRows.map((row, ri) => (
                            <tr
                                key={ri}
                                className={`border-b border-gray-800/40 ${ri % 2 === 1 ? 'bg-[#131322]' : 'bg-[#0f0f1a]'} hover:bg-[#1a1a2e] transition-colors`}
                            >
