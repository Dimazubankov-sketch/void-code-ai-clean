...
export function TypewriterMessage({ content, onProgress, onDone }) {
    ...
    useEffect(() => {
        ...
        // Длительность подстраивается под длину: короткие реплики
        // печатаются заметно, длинные не тянутся дольше ~1.2с. Прежние
        // 2.2с ощущались вяло на длинных ответах — сократили ради
        // отзывчивости, сохранив «эффект печати».
        const duration = Math.min(1.2, Math.max(0.25, text.length * 0.005));
