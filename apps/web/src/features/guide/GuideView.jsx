...
export const GUIDE_SECTIONS = [
    {
        ...
        items: [
            ...
            'Модель выбирается вверху экрана чата: Void Mini — быстрая и полностью безлимитная; Void Plus и Void Pro — мощные модели для кода и сложных задач (расходуют дневной лимит запросов)',
            'Void Plus и Void Pro специально заточены под написание кода и глубоко прорабатывают решения',
    ...
    {
        id: 'voice', icon: 'Waveform', color: 'purple', title: 'Голосовой режим',
        desc: 'Разговор с ИИ вслух: говорите как с человеком, а он отвечает голосом почти сразу.',
        items: [
            'Открывается кнопкой с иконкой волны справа в поле ввода — она появляется, когда поле пустое',
            'Нажимать ничего не нужно: микрофон слушает сразу, пауза в речи означает, что вы договорили',
            'Ответ начинает звучать ещё до того, как модель допишет его целиком — фраза за фразой',
            'Перебить можно голосом: начните говорить, и ИИ плавно затихнет примерно за секунду',
            'Кнопка «+» включает камеру или демонстрацию экрана — ИИ увидит, о чём идёт речь',
            'Тап по кругу сворачивает режим: видно всю беседу текстом, повторный тап разворачивает',
            'Кнопка с ползунками — голос, «личность» собеседника, язык, скорость и проверка микрофона',
            'Из Хаба разговор начинается в новом чате, из переписки — продолжается в ней',
            'На платных тарифах разговор ведёт премиальная модель более высокого класса: точнее понимает формулировки, лучше держит контекст длинной беседы и даёт содержательные развёрнутые ответы',
            'На Ultra на сложных темах автоматически подключается режим углублённых рассуждений, а простые вопросы остаются быстрыми'
        ]
    },
    {
        id: 'images', icon: 'Image', color: 'pink', title: 'Генерация изображений',
        desc: 'Опишите идею словами — получите настоящее изображение прямо в чате.',
        items: [
            'Запускается пунктом «Создать изображение» в меню «+» поля ввода — включается режим изображения (иконка с крестиком над полем)',
            'Опишите словами, что хотите увидеть — чем подробнее описание, тем точнее результат',
            'Изображение генерируется на серверной стороне и приходит прямо в переписку',
            'Пока картинка создаётся, показывается плавная анимация генерации на GSAP',
            'Готовое изображение можно скачать кнопкой загрузки прямо из чата',
            'Все изображения автоматически сохраняются в Библиотеку'
        ]
    },
    {
    ...
    },
    {
        ...
        desc: 'Объединяйте чаты в проекты со сквозным контекстом, скиллами и инструкциями.',
        items: [
            'Открываются из бокового меню — кнопка «Проекты»',
            'Внутри проекта три подвкладки: «Чаты», «Скиллы» и «Инструкции»',
            'Подвкладка «Скиллы»: скиллы, добавленные в проекте, работают только в его чатах (изолированы от остальных)',
            'Подвкладка «Инструкции» (память проекта): здесь хранится ключевой контекст — стек, архитектура, важные детали и ключи; есть поиск по документу',
            'Тоггл «Сквозной контекст проекта» (по умолчанию включён): пока включён, ключевая информация из чатов проекта автоматически пополняет «Инструкции», и новый чат проекта считывает весь этот контекст',
            'Если тоггл выключен — запись новых данных в инструкции приостанавливается, но существующий контекст продолжает учитываться'
        ]
    },
    {
        id: 'skills', icon: 'Skills', color: 'indigo', title: 'Скиллы',
        desc: 'Инструкции, улучшающие ответы: код, анимации, архитектура и другое.',
        items: [
            'Открываются пунктом «Скиллы» в боковом меню и в меню «+» поля ввода (под «Добавить в проект»)',
            'Вкладка «Базовые скиллы» — встроенные наборы инструкций Void Code AI, включаются переключателем',
            'Вкладка «Свои скиллы» — кастомные скиллы: добавьте их текстом (название, описание, инструкция) или импортом из GitHub',
            'Импорт из GitHub: введите personal access token (для приватных репозиториев) или имя пользователя (для публичных), выберите репозиторий — его README подтянется как инструкция скилла',
            'Активные скиллы автоматически учитываются ассистентом при ответах во всех чатах',
            'Внутри проекта скиллы изолированы — работают только в чатах этого проекта'
        ]
    },
    {
    ...
    },
    {
        ...
        items: [
            'Пополнить баланс можно банковской картой (в том числе сканером через камеру) или через СБП',
    ...
    },
    {
        ...
        desc: 'Free, Pro и Ultra — разный объём запросов к мощным моделям.',
        items: [
            'Free, Pro и Ultra — чем выше тариф, тем больше дневных и недельных запросов к мощным моделям',
            'Оркестраторы: на Pro можно купить до 3, на Ultra — до 5',
            'Модель Flash всегда бесплатна и безлимитна, независимо от тарифа',
            'При исчерпании дневного лимита остаётся доступна модель Flash, а лимиты автоматически восстанавливаются через 6 часов',
            'Оплатить подписку можно картой, СБП или прямо с баланса кошелька',
