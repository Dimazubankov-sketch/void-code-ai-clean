...
export const GUIDE_SECTIONS = [
    {
        ...
        items: [
            ...
            'Модель выбирается вверху экрана чата: Flash — быстрая и полностью безлимитная; Flash Расширенный и Void Code Pro — для сложных задач (расходуют дневной лимит запросов)',
    ...
    {
        id: 'agents', icon: 'Robot', color: 'indigo', title: 'Конструктор AI-агентов',
        desc: 'Соберите личного AI-агента — визуально по блокам или через диалог-мастер.',
        items: [
            'Два способа собрать агента: по блокам на визуальном канвасе или через диалог с помощником, который проведёт по шагам',
            'По блокам: добавьте нужные блоки (Триггеры, Действия, AI) и соедините их — потяните за кружок справа от блока к другому блоку',
            'Тап по блоку открывает его настройки; чтобы переставить блок — просто зажмите и потяните (на телефоне настройки больше не мешают перетаскиванию)',
            'На телефоне блоки добавляются круглой кнопкой «+» внизу справа; щипок двумя пальцами масштабирует и панорамирует канвас',
            'На компьютере блоки берутся из левой панели, а колесо мыши зумирует канвас',
            'Кнопки отмены/повтора и «Показать всё» помогают не потеряться; замок фиксирует канвас от случайных сдвигов',
            'Кнопка «Тестировать» прогоняет агента по цепочке блоков, наглядно показывая порядок выполнения',
            'Через чат: нажмите «Кастомные агенты» и ответьте на несколько простых вопросов — агент соберётся автоматически',
            'История всех агентов — в меню конструктора (значок с двумя полосками): там можно переименовать, удалить или вернуться к прошлому агенту',
            'Сборка агента — разовая платная услуга; дальнейшая работа агента списывает токены с баланса кошелька'
        ]
    },
    {
        id: 'cockpit', icon: 'RobotArmy', color: 'purple', title: 'Cockpit: управление агентами',
        desc: 'Пульт управления всеми агентами и оркестраторами: статусы, задачи, привязка подчинённых.',
        items: [
            'Открывается карточкой «Управление агентами» на главном экране или пунктом «Cockpit» в боковом меню — работает в связке с конструктором агентов',
            'Все агенты, собранные в конструкторе, автоматически появляются в Cockpit — отдельно ничего добавлять не нужно',
            'Каждый агент показывает визуальный статус: «Активен» (зелёный), «В задаче» (жёлтый) или «Спит» (серый); неоплаченные агенты помечаются и считаются спящими, пока не оплачены',
            'Сверху — поиск по названию: быстро находит нужного агента среди списков',
            'Клик по агенту открывает мини-панель действий: запустить или остановить, принудительный запуск, редактировать логику, правка через промпт, переименовать и настроить график работы',
            'Правка через промпт — «прошивка» инструкций текстом поверх блоков: опишите словами, как агент должен себя вести; история прошивок сохраняется',
            'Оркестратор — особый тип агента-дирижёра: он раздаёт задачи другим агентам. Создаётся в конструкторе на вкладке «Оркестраторы», стоит дороже обычного агента',
            'У каждого оркестратора своя уникальная почта вида agent_000000@voidops.ai — она выдаётся при создании и не редактируется',
            'В чате с оркестратором кнопка со счётчиком открывает меню привязки: слева оркестратор, справа — список агентов с поиском; можно смотреть только свободных или всех, а занятые другим оркестратором показаны тускло с пометкой, к кому привязаны',
            'Поставьте оркестратору задачу словами — он разложит её на подзадачи и предложит план, кому что поручить',
            'Протокол доверия: оркестратор не меняет инструкции подчинённых сам — сначала присылает отчёт с планом и тремя кнопками «Разрешить», «Отредактировать», «Отказаться». Задачи раздаются только после вашего подтверждения',
    ...
    },
    {
        ...
        items: [
            ...
            'Два способа собрать сайт: вручную из блоков на визуальном макете или через чат-мастера — карточка «Не хочется собирать блоки? Соберём в чате» на экране конструктора',
            'Ручная сборка: кнопка «Создать макет сайта» открывает настоящий визуальный макет сайта; слева — библиотека блоков (обложка, услуги, галерея, форма и др.), их перетаскивают прямо на макет (на телефоне — просто нажать на блок в списке снизу)',
