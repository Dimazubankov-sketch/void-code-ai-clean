...
import { EmailAgentMailboxes } from '@/features/agents/EmailAgentMailboxes';
import { OrchestratorPurchaseModal } from '@/features/agents/OrchestratorPurchaseModal';
import { getOrchestratorLimit, resolveCockpitStatus } from '@/shared/config/orchestrator';
...
export function AgentBuilderMenu({ open, onClose, agents, activeAgentId, onSelectAgent, onNewAgent, onDeleteAgent, onRenameAgent, updateState, state }) {
    ...
    const [mailboxAgent, setMailboxAgent] = useState(null);
        ...
        <>
            ...
            <div className={`fixed top-0 right-0 h-full w-[85vw] max-w-sm bg-white dark:bg-darkCard z-50 shadow-2xl transition-transform duration-300 flex flex-col ${open ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="px-5 flex-1 overflow-y-auto pb-6">
                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 ml-1">{tab === 'orchestrators' ? 'Список оркестраторов' : 'Список агентов'}</h4>
                    <p className="text-[11px] text-gray-400 mb-3 ml-1">Оплаченных можно перетащить на холст</p>
                    {sorted.length === 0 ? (
                        ...
                        <div className="space-y-2">
                            {sorted.map(agent => (
                                <div key={agent.id} draggable={agent.isPaid} onDragStart={(e) => { if (agent.isPaid) { e.dataTransfer.setData('application/x-agent-id', agent.id); e.dataTransfer.effectAllowed = 'copy'; } }} className={`group p-3 rounded-2xl border transition-colors ${agent.isPaid ? 'cursor-grab active:cursor-grabbing' : ''} ${activeAgentId === agent.id ? 'bg-[#efecf9] dark:bg-purple-900/20 border-[#5b32d4]/30' : 'bg-gray-50 dark:bg-gray-800/40 border-transparent hover:border-gray-200 dark:hover:border-gray-700'}`}>
                                    {renamingId === agent.id ? (
                                        ...
                                        <div className="flex items-center gap-2">
                                            {/* Статус-кружок: активен (зелёный) / в задаче (жёлтый) / спит (серый) / ждёт оплаты (красный) */}
                                            ...
                                            <button onClick={() => agent.kind === 'orchestrator' ? updateState({ activeAgentId: agent.id, currentView: 'orchestrator-chat' }) : onSelectAgent(agent)} className="flex-1 min-w-0 text-left">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    {!agent.isPaid && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-red-50 dark:bg-red-900/20 text-red-500 flex-shrink-0">Ждёт оплаты</span>}
                                                </div>
                                            ...
                                            {agent.storeId === 'email_agent' && (
                                                <button onClick={() => setMailboxAgent(agent)} className="void-tap-target flex-shrink-0 p-2 text-gray-400 hover:text-[#5b32d4] hover:bg-white dark:hover:bg-gray-800 rounded-lg transition-colors" title="Почты агента"><Icons.Mail className="w-4 h-4" /></button>
                                            )}
            ...
            {mailboxAgent && (
                <EmailAgentMailboxes
                    state={state}
                    updateState={updateState}
                    agent={(state.aiAgents || []).find(a => a.id === mailboxAgent.id) || mailboxAgent}
                    onClose={() => setMailboxAgent(null)}
                />
            )}
        </>
    );
...
// Статус-кружок агента:
//   ждёт оплаты — красный (агент не куплен)
//   активен — зелёный, в задаче — жёлтый, спит — серый
function StatusDot({ agent }) {
    let color = '#94a3b8'; // спит (серый) по умолчанию
    let title = 'Спит';
    if (agent.isPaid === false) { color = '#ef4444'; title = 'Ждёт оплаты'; }
    else {
        const st = resolveCockpitStatus(agent);
        if (st === 'active') { color = '#22c55e'; title = 'Активен'; }
        else if (st === 'working') { color = '#f59e0b'; title = 'В задаче'; }
        else { color = '#94a3b8'; title = 'Спит'; }
    }
    return <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color }} title={title} />;
}
