...
export function StoreCard({ item, premium = false, owned = false, onOpen, index = 0 }) {
    ...
    return (
        <button
            ...
            style={{ animationDelay: `${index * 70}ms`, ...(premium ? { border: '2px solid transparent', backgroundImage: 'linear-gradient(var(--tw-bg-opacity, white), var(--tw-bg-opacity, white)), linear-gradient(135deg, #312a6b, #3f4dab, #a52fe0)', backgroundOrigin: 'border-box', backgroundClip: 'padding-box, border-box' } : {}) }}
        >
