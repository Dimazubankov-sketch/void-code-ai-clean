...
export function AgentStoreApp({ state, updateState }) {
    ...
    const buyOrchestrator = () => {
        // Оркестраторы доступны только с тарифа Pro: Pro — РОВНО ОДИН,
        // Ultra — до 3. На Plus оркестраторы больше не продаются.
        if (!canUseOrchestrators(plan)) {
            setLimitNotice({
                title: 'Оркестраторы недоступны',
                text: 'Оркестраторы доступны с тарифа Pro (один оркестратор) и Ultra (до трёх). Обновите тариф, чтобы покупать и использовать оркестраторов.',
            });
            return;
        }
