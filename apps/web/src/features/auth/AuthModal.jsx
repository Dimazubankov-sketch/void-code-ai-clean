...
export function AuthModal({ state, updateState }) {
    ...
    const handleAuth = () => {
        ...
        // Домен фиксирован — собираем полный адрес из имени пользователя
        const fullEmail = `${username}@voidops.com`;
        setHasError(false);
        setErrorMsg('');
        const key = fullEmail;
        const accountPlans = state.accountPlans || {};
        const isNewAccount = state.authTab === 'register';
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-start sm:items-center justify-center p-3 sm:p-4 overflow-y-auto fade-in">
