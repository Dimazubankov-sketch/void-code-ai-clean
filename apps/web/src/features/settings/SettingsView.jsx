...
export function SettingsView({ state, updateState }) {
    ...
    // снова открыть панель «Личные данные», а не просто показать голые
    // Настройки — она была модалкой поверх этого экрана, а не отдельным view.
    useEffect(() => {
        if (state.reopenAccountsPanel) {
            setShowAccounts(true);
            updateState({ reopenAccountsPanel: false });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [state.reopenAccountsPanel]);
        ...
        <div className="flex-1 overflow-y-auto pb-12 h-full bg-[#f8f9fc] dark:bg-darkBg fade-in w-full">
            <div className="max-w-2xl mx-auto px-4 py-8 md:py-12">
                <div className="flex items-center mb-8 gap-4">
                    ...
                    <h2 className="text-3xl font-extrabold dark:text-white">{t(lang, 'settings.title')}</h2>
                </div>
