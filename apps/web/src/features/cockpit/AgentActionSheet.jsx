...
export function AgentActionSheet({ agent, state, updateState, onClose }) {
    ...
    const toggleRun = () => {
        if (!canRun) { window.alert('Агент не оплачен. Купите его в магазине агентов, чтобы запускать.'); return; }
    ...
    // Редактирование: только оркестратор открывает чат с собой —
    // у обычных агентов логика правится через промпт (кнопка ниже),
    // отдельного конструктора больше нет.
    const editOrchestrator = () => {
            ...
            >
                {mode === 'menu' && (
                    <div className="p-3">
                        ...
                        {isOrch && <Action icon={Icons.Pencil} label="Открыть чат оркестратора" onClick={editOrchestrator} />}
