...
// Задача 4 (уточнение): свёрнутый рельс RightMenu на ПК показывается
// ТОЛЬКО в режиме чата. Во всех остальных вкладках (изображения, проекты,
// навыки, плагины, библиотека, агенты, настройки и т.д.) рельса нет —
// у каждой из них своя навигация (кнопка «Назад», левый сайдбар магазина
// агентов, переключатель «Чат» в Image Studio). Единый источник правды и
...
export function App() {
    ...
    const [state, setState] = useState(() => {
        ...
        // ОБНОВЛЕНО: раньше здесь модалка входа открывалась СРАЗУ и
        // принудительно для любого нового посетителя (см. историю правок).
        // Теперь наоборот — гость сначала видит обычный интерфейс Void
        // Code (после заставки), а кнопка меню в шапке заменяется на
        // «Войти» (см. HomeView.jsx). Модалка входа/регистрации всплывает
        // только когда гость пытается воспользоваться функцией, которая
        // требует аккаунта: отправить сообщение в чат, записать голос,
        // войти в Voice Mode, открыть Cockpit или почту — везде эти
        // действия точечно проверяют state.user и сами вызывают
        // showAuthModal: true. showAuthModal ниже больше НЕ форсится на
        // старте — mandatory-режим в AuthModal.jsx (без крестика) по-прежнему
        // работает, просто модалка теперь не выскакивает без действия
        // пользователя.
        if (!saved) return { ...defaults, showAuthModal: false };
        ...
        return {
            ...
            customSkills: [],
            isGeneratingImage: false,
            // ОБНОВЛЕНО (см. комментарий выше про !saved): модалка больше
            // не форсится для гостя на старте — интерфейс виден сразу,
            // модалка появляется точечно на гейтнутых действиях.
            showAuthModal: false,
