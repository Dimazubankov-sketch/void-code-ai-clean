...
export const generateMockReply = (messages, systemPrompt, modelId) => {
    ...
    const isCodeIntent = MOCK_CODE_WORDS.some(w => text.includes(w));

    // Void Plus и Void Pro — обе модели умеют писать код: при запросе на код
    // они отвечают готовым примером с живым предпросмотром (окно просмотра
    // кода). Void Pro решает более сложные задачи, Void Plus — задачи попроще,
    // но в тестовом режиме демонстрация одинаковая для обеих моделей.
    if ((modelId === 'pro' || modelId === 'flash_ext') && isCodeIntent) {
