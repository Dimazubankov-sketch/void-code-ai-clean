...
export class ImageService {
  ...
  async generate(prompt: string, images?: string[]): Promise<string> {
    ...
    // Раньше при наличии референса ВСЕГДА уходила инструкция «сохрани
    // исходное фото без изменений». Это ломало ровно противоположный
    // сценарий: пользователь просит «создай похожее / в таком же стиле /
    // такую же, но другую» — а модель обязана была вернуть то же самое
    // фото. Поэтому теперь различаем два намерения по формулировке
    // запроса: РЕДАКТИРОВАНИЕ (по умолчанию при референсе) и ПОХОЖЕЕ.
    const SIMILAR_INTENT =
